    ...
    return (
      <form.Field name={`questions[${questionIndex}].options`} mode="array">
        {(optionsField) => {
          ...
          return (
            <FieldGroup>
              {/* correctValue drives the RadioGroup's controlled selection.
                  It must come from form.Subscribe (not a local const) so it
                  reactively updates when isCorrect changes via setFieldValue. */}
              <form.Subscribe
                selector={(state) => {
                  const question = state.values.questions[questionIndex]
                  if (!question || question.type !== "SINGLE_SELECT") return ""
                  return question.options.find((o) => o.isCorrect)?.value ?? ""
                }}
              >
                {(correctValue) => (
                  <RadioGroup value={correctValue} onValueChange={handleCorrectChange}>
                            ...
                            >
                              {({ liveValue, isDuplicate, isCorrect, order }) => (
                                <FieldGroup className="flex flex-row items-baseline">
                                  ...
                                  {/* RadioGroupItem must read liveValue (from Subscribe),
                                      not option.value (from the map closure), otherwise
                                      it lags one render behind when the label is typed. */}
                                  <RadioGroupItem
                                    value={liveValue}
                                    disabled={liveValue === ""}
                                  ...
                                  <Field data-invalid={labelIsInvalid || isDuplicate}>
                                    <Input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        // the new slug before handleCorrectChange reads it.
